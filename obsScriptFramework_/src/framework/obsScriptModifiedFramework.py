...
class ModifiedFunction:
    ...
    def property_modified(self, control_name:str, modified_callback_name:str) -> Callable[[Any, Any, Any], bool]:
        def build_pm(ps, p, st=None):
            self.ObsScriptGlobalVariable.Log_manager.log_info(f"监测到控件变动: {control_name}")
            if control_name == "e58581e8aeb8e689a7e8a18ce68ea7e4bbb6e4bfaee694b9e59b9ee8b083":
                ...
                try:
                    getattr(self.BtnFunction, modified_callback_name)(control_name=control_name)
                except AttributeError:
                    self.ObsScriptGlobalVariable.Log_manager.log_error(f"未找到【{control_name}】对应的控件变动回调函数")
            return True
        return build_pm