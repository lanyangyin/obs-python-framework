...
class Build:
    ...
    def _create_textbox(self, w) -> None:
        """
        创建文本框控件
    ...
    def _create_button(self,w) -> None:
        """
        创建按钮控件
    ...
    def _create_combobox(self,w) -> None:
        """
        创建组合框（下拉列表）控件
    ...
    def _create_pathbox(self,w) -> None:
        """
        创建路径选择框控件
    ...
    # --- 4. 辅助函数：获取创建器 ---
    def get_control_creator(self,control_type: WidgetCategory):
        """根据控件类型字符串获取对应的创建函数"""
        try:
    ...
    def _init_property_sets(self,widgets):
        """职责1：初始化所有需要的属性集"""
    ...
    def _create_control_for_widget(self,w):
        """为单个控件对象执行创建和通用设置"""
        # 1. 获取对应的创建函数
        creator = self.get_control_creator(w.WidgetType)
        if not creator:
